...
async function setupMyToken() {
    ...
    console.log(`Minting ${mintAmount.toString()} tokens to ${acc1.account.address} confirmed, current block number: ${mintTx1BlockNumber}`);
    await MyToken.write.delegate([acc1.account.address], { account: acc1.account });


    await MyToken.write.mint([acc2.account.address, mintAmount]);
    const mintTx2BlockNumber = await publicClient.getBlockNumber();
    console.log(`Minting ${mintAmount.toString()} tokens to ${acc2.account.address} confirmed, current block number: ${mintTx2BlockNumber}`);
    await MyToken.write.delegate([acc2.account.address], { account: acc2.account });

    // Mine blocks after minting to simulate time passing
    await mineBlocks(10);

    const latestBlockNumber = await publicClient.getBlockNumber();
    console.log(`Latest block number after mining additional blocks: ${latestBlockNumber}`);

    return { MyToken, acc1, acc2, latestBlockNumber };
}
...
async function setupTokenizedBallot(MyToken, snapshotBlockNumber) {
    ...
    console.log(`Snapshot block number that is passed to TokenizedBallot as target block number: ${snapshotBlockNumber}`);

    return TokenizedBallot;
}

// Get the token balances at the snapshot block
async function logPastTokenBalances(tokenContract, accountAddresses, snapshotBlock) {
    for (const address of accountAddresses) {
        const pastBalance = await tokenContract.read.getTokenBalanceAtBlock([address, snapshotBlock]);
        console.log(`Address ${address} had ${pastBalance.toString()} balance at block ${snapshotBlock}.`);
    }
}


describe("Token and Voting System Tests", function () {
    let MyToken, TokenizedBallot, acc1, acc2, snapshotBlockNumber;
    ...
    before(async function () {
        ...
        const latestBlockNumber = setupMyTokenResult.latestBlockNumber;

        // Setting the snapshot block number to deployment block - 3
        // Assuming the "deployment block" as the current latest block number for TokenizedBallot
        snapshotBlockNumber = latestBlockNumber - BigInt(3);
        ...
        await logPastTokenBalances(MyToken, [acc1.account.address, acc2.account.address], snapshotBlockNumber);
    ...
    it("acc1: should allow voting", async function () {
        // acc1 votes
        await TokenizedBallot.write.vote([1, parseEther("50")], { account: acc1.account });
        // Further checks can be added to verify the vote
    });

    it("acc2: should allow voting", async function () {
        // acc2 votes
        await TokenizedBallot.write.vote([1, parseEther("50")], { account: acc2.account });
        // Further checks can be added to verify the vote
    });
